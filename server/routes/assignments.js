...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const rows = await query(`SELECT a.id, a.vehicle_id, a.driver_id, a.start_date, a.end_date,
...
router.get('/vehicle/:vehicleId', requireIdParam('vehicleId'), asyncHandler(async (req, res) => {
    const limit = Math.min(Math.max(parseInt(req.query.limit) || 200, 1), 1000);
    const rows = await query(`SELECT ${COLUMNS_SQL} FROM vehicle_assignments WHERE vehicle_id = $1 ORDER BY created_at DESC LIMIT $2`, [req.params.vehicleId, limit]);
    res.json(rows);
}));
// GET /api/assignments/driver/:driverId
router.get('/driver/:driverId', requireIdParam('driverId'), asyncHandler(async (req, res) => {
    const limit = Math.min(Math.max(parseInt(req.query.limit) || 200, 1), 1000);
    const rows = await query(`SELECT ${COLUMNS_SQL} FROM vehicle_assignments WHERE driver_id = $1 ORDER BY created_at DESC LIMIT $2`, [req.params.driverId, limit]);
    res.json(rows);
}));
// POST /api/assignments — create assignment
router.post('/', requireRole('admin', 'manager'), requireFields(['vehicleId', 'driverId', 'startDate']), asyncHandler(async (req, res) => {
    const b = req.body;
    const id = randomUUID();
    const created = await executeReturning(`INSERT INTO vehicle_assignments (id, vehicle_id, driver_id, start_date, end_date, purpose, status, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING ${COLUMNS_SQL}`, [id, b.vehicleId, b.driverId, b.startDate, b.endDate ?? null, b.purpose ?? '', b.status ?? 'active', b.notes ?? '']);
...
router.put('/:id', requireRole('admin', 'manager'), requireIdParam(), asyncHandler(async (req, res) => {
    ...
    if (fields.length > 0) {
        ...
        const updated = await executeReturning(`UPDATE vehicle_assignments SET ${fields.join(', ')} WHERE id = $${paramIndex}
