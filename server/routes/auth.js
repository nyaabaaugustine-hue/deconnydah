...
router.post('/login', asyncHandler(async (req, res) => {
    ...
    const user = await queryOne('SELECT id, username, password_hash, display_name, role, must_change_password FROM admin_users WHERE username = $1', [username]);
    ...
    await query('INSERT INTO sessions (id, user_id, username, token, expires_at) VALUES ($1, $2, $3, $4, $5)', [crypto.randomUUID(), user.id, user.username, token, expiresAt.toISOString()]);
    res.json({
        token,
        user: {
            id: user.id,
...
router.get('/me', asyncHandler(async (req, res) => {
    ...
    const fullUser = await queryOne('SELECT id, username, display_name, role, must_change_password FROM admin_users WHERE id = $1', [auth.userId]);
    if (!fullUser) {
...
router.post('/change-password', requireAuth, asyncHandler(async (req, res) => {
    ...
    const admin = await queryOne('SELECT id, password_hash FROM admin_users WHERE id = $1', [user.userId]);
...
router.get('/users', requireAuth, requireRole('admin'), asyncHandler(async (_req, res) => {
    const users = await query('SELECT id, username, display_name, role, created_at FROM admin_users ORDER BY created_at ASC');
...
router.post('/users', requireAuth, requireRole('admin'), asyncHandler(async (req, res) => {
    ...
    await query('INSERT INTO admin_users (id, username, password_hash, display_name, role, must_change_password) VALUES ($1, $2, $3, $4, $5, $6)', [id, username, hash, displayName, userRole, true]);
...
router.patch('/users/:id', requireAuth, requireRole('admin'), asyncHandler(async (req, res) => {
    ...
    const user = await queryOne('SELECT id FROM admin_users WHERE id = $1', [id]);
    if (!user) {
