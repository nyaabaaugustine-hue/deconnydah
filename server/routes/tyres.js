...
router.get('/vehicle/:vehicleId', requireIdParam('vehicleId'), asyncHandler(async (req, res) => {
    ...
    const logs = await query(`SELECT ${COLUMNS_SQL} FROM tyre_logs WHERE vehicle_id = $1 ORDER BY install_date DESC LIMIT $2`, [req.params.vehicleId, limit]);
    res.json(logs);
}));
// POST /api/tyres
router.post('/', requireRole('admin', 'manager'), requireFields(['vehicleId', 'position', 'installDate', 'brand', 'cost']), asyncHandler(async (req, res) => {
    const b = req.body;
    ...
    const id = randomUUID();
    const created = await executeReturning(`INSERT INTO tyre_logs (id, vehicle_id, position, install_date, replacement_date, brand, cost)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING ${COLUMNS_SQL}`, [id, b.vehicleId, b.position, b.installDate, b.replacementDate ?? null, b.brand, b.cost]);
...
router.patch('/:id', requireRole('admin', 'manager'), requireIdParam(), asyncHandler(async (req, res) => {
    ...
    const updated = await executeReturning(`UPDATE tyre_logs SET ${updates.join(', ')} WHERE id = $${idx} RETURNING ${COLUMNS_SQL}`, params);
    if (!updated) {
