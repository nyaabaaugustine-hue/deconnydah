...
router.post('/', requireRole('admin', 'manager'), requireFields(['name']), asyncHandler(async (req, res) => {
    const b = req.body;
    const id = randomUUID();
    const created = await executeReturning(`INSERT INTO service_providers (id, name, type, phone, email, address, specialties, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING ${COLUMNS_SQL}`, [id, b.name, b.type ?? null, b.phone ?? null, b.email ?? null, b.address ?? null, b.specialties ?? null, b.notes ?? null]);
...
router.put('/:id', requireRole('admin', 'manager'), requireIdParam(), asyncHandler(async (req, res) => {
    ...
    if (fields.length > 0) {
        ...
        updated = await executeReturning(`UPDATE service_providers SET ${fields.join(', ')}, updated_at = NOW() WHERE id = $${param}
         RETURNING ${COLUMNS_SQL}`, [...values, req.params.id]);
    }
    res.json(updated);
}));
