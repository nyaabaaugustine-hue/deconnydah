...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const rows = await query(`SELECT dl.*, d.full_name AS driver_name FROM driver_licenses dl JOIN drivers d ON d.id = dl.driver_id ORDER BY dl.expiry_date DESC LIMIT $1`, [limit]);
    res.json(rows);
}));
// GET /api/driver-licenses/driver/:driverId — list for one driver
router.get('/driver/:driverId', asyncHandler(async (req, res) => {
    const limit = Math.min(Math.max(parseInt(req.query.limit) || 200, 1), 1000);
    const rows = await query(`SELECT dl.*, d.full_name AS driver_name FROM driver_licenses dl JOIN drivers d ON d.id = dl.driver_id WHERE dl.driver_id = $1 ORDER BY dl.expiry_date DESC LIMIT $2`, [req.params.driverId, limit]);
    res.json(rows);
}));
// POST /api/driver-licenses — create
router.post('/', requireRole('admin', 'manager'), requireFields(['driverId', 'licenseNumber', 'issueDate', 'expiryDate']), asyncHandler(async (req, res) => {
    const b = req.body;
    const id = randomUUID();
    const created = await executeReturning(`INSERT INTO driver_licenses (id, driver_id, license_class, license_number, issue_date, expiry_date, issuing_authority, status, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING ${COLUMNS_SQL}`, [id, b.driverId, b.licenseClass ?? null, b.licenseNumber, b.issueDate, b.expiryDate, b.issuingAuthority ?? null, b.status ?? null, b.notes ?? null]);
...
router.put('/:id', requireRole('admin', 'manager'), requireIdParam(), asyncHandler(async (req, res) => {
    ...
    if (fields.length > 0) {
        ...
        const updated = await executeReturning(`UPDATE driver_licenses SET ${fields.join(', ')} WHERE id = $${paramIndex}
