...
router.get('/vehicle/:vehicleId', requireIdParam('vehicleId'), asyncHandler(async (req, res) => {
    ...
    const photos = await query(`SELECT ${COLUMNS_SQL} FROM vehicle_photos WHERE vehicle_id = $1 ORDER BY taken_at DESC LIMIT $2`, [req.params.vehicleId, limit]);
    res.json(photos);
}));
// POST /api/photos
...
router.post('/', requireRole('admin', 'manager'), requireFields(['vehicleId', 'category', 'caption', 'takenAt', 'imageUrl']), asyncHandler(async (req, res) => {
    const b = req.body;
    const id = randomUUID();
    const created = await executeReturning(`INSERT INTO vehicle_photos (id, vehicle_id, category, caption, taken_at, image_url)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING ${COLUMNS_SQL}`, [id, b.vehicleId, b.category, b.caption, b.takenAt, b.imageUrl]);
...
router.patch('/:id', requireRole('admin', 'manager'), requireIdParam(), asyncHandler(async (req, res) => {
    ...
    if (b.caption !== undefined) {
        ...
        params.push(b.caption);
    }
    if (b.takenAt !== undefined) {
        updates.push(`taken_at = $${idx++}`);
        params.push(b.takenAt);
    }
    if (b.imageUrl !== undefined) {
        updates.push(`image_url = $${idx++}`);
        params.push(b.imageUrl);
    }
    if (updates.length === 0) {
    ...
    const updated = await executeReturning(`UPDATE vehicle_photos SET ${updates.join(', ')} WHERE id = $${idx} RETURNING ${COLUMNS_SQL}`, params);
    if (!updated) {
