...
router.get('/low-stock', asyncHandler(async (req, res) => {
    const limit = Math.min(Math.max(parseInt(req.query.limit) || 200, 1), 1000);
    const rows = await query(`SELECT ${COLUMNS_SQL} FROM spare_parts WHERE quantity <= min_quantity ORDER BY name LIMIT $1`, [limit]);
    res.json(rows);
}));
// POST /api/spare-parts — create part
router.post('/', requireRole('admin', 'manager'), requireFields(['name']), asyncHandler(async (req, res) => {
    const b = req.body;
    const id = randomUUID();
    const created = await executeReturning(`INSERT INTO spare_parts (id, name, part_number, category, quantity, min_quantity, unit_cost, supplier, location)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING ${COLUMNS_SQL}`, [id, b.name, b.partNumber ?? null, b.category ?? null, b.quantity ?? 0, b.minQuantity ?? 0, b.unitCost ?? null, b.supplier ?? null, b.location ?? null]);
...
router.put('/:id', requireRole('admin', 'manager'), requireIdParam(), asyncHandler(async (req, res) => {
    ...
    if (fields.length > 0) {
        ...
        const updated = await executeReturning(`UPDATE spare_parts SET ${fields.join(', ')} WHERE id = $${paramIndex}
