...
router.get('/:id', requireIdParam(), asyncHandler(async (req, res) => {
    const driver = await queryOne(`SELECT ${COLUMNS_SQL} FROM drivers WHERE id = $1 AND deleted_at IS NULL`, [req.params.id]);
    if (!driver) {
...
router.get('/:id/profile', requireIdParam(), asyncHandler(async (req, res) => {
    const driverId = req.params.id;
    const driver = await queryOne(`SELECT ${COLUMNS_SQL} FROM drivers WHERE id = $1 AND deleted_at IS NULL`, [driverId]);
    if (!driver) {
        res.status(404).json({ error: 'Driver not found' });
        return;
    }
    const [supervisor, assignedVehicle, inspections, revenueList, accidentReports, revenueAgg] = await Promise.all([
        // Supervisor info
        queryOne(`SELECT id, full_name, phone, region FROM supervisors WHERE id = $1 AND deleted_at IS NULL`, [driver.supervisor_id]),
        // Assigned vehicle
        queryOne(`SELECT id, plate_number, make, model, year, status FROM vehicles WHERE current_driver_id = $1 AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 1`, [driverId]),
        // Inspections where this driver participated
        query(`SELECT id, vehicle_id, inspection_date, overall_status, notes, photo_count FROM inspections WHERE driver_name = $1 ORDER BY inspection_date DESC LIMIT 20`, [driver.full_name]),
        // Revenue entries
        query(`SELECT id, vehicle_id, trip_date, trip_reference, route, client, amount FROM revenue_entries WHERE driver_id = $1 ORDER BY trip_date DESC LIMIT 20`, [driverId]),
        // Accident reports
        query(`SELECT id, vehicle_id, accident_date, description, cost, driver_at_fault FROM accident_reports WHERE driver_id = $1 ORDER BY accident_date DESC LIMIT 20`, [driverId]),
...
router.post('/', requireRole('admin', 'manager'), requireFields(['fullName', 'phone', 'licenseNumber', 'hireDate']), asyncHandler(async (req, res) => {
    const b = req.body;
    const id = randomUUID();
    const created = await executeReturning(`INSERT INTO drivers (id, full_name, phone, license_number, supervisor_id, hire_date, status, photo_url)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING ${COLUMNS_SQL}`, [id, b.fullName, b.phone, b.licenseNumber, b.supervisorId ?? null, b.hireDate, b.status ?? 'active', b.photoUrl ?? null]);
    res.status(201).json(created);
}));
// PATCH /api/drivers/:id
router.patch('/:id', requireRole('admin', 'manager'), requireIdParam(), asyncHandler(async (req, res) => {
    const existing = await queryOne(`SELECT ${COLUMNS_SQL} FROM drivers WHERE id = $1 AND deleted_at IS NULL`, [req.params.id]);
    if (!existing) {
        res.status(404).json({ error: 'Driver not found' });
        return;
    }
    ...
    if (fields.length > 0) {
        ...
        updated = await executeReturning(`UPDATE drivers SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING ${COLUMNS_SQL}`, values);
    }
    else {
        updated = existing;
    }
...
router.delete('/:id', requireRole('admin'), requireIdParam(), asyncHandler(async (req, res) => {
    const result = await execute(`UPDATE drivers SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL`, [req.params.id]);
