...
router.get('/vehicle/:vehicleId', requireIdParam('vehicleId'), asyncHandler(async (req, res) => {
    const limit = Math.min(Math.max(parseInt(req.query.limit) || 200, 1), 1000);
    const rows = await query(`SELECT ${COLUMNS_SQL} FROM inspections WHERE vehicle_id = $1 ORDER BY inspection_date DESC LIMIT $2`, [req.params.vehicleId, limit]);
    res.json(rows);
}));
// GET /api/inspections/:id
router.get('/:id', requireIdParam(), asyncHandler(async (req, res) => {
    const row = await queryOne(`SELECT ${COLUMNS_SQL} FROM inspections WHERE id = $1`, [req.params.id]);
    if (!row) {
...
router.post('/', requireRole('admin', 'manager'), requireFields(['vehicleId', 'driverName', 'inspectionDate', 'overallStatus', 'checklist']), asyncHandler(async (req, res) => {
    const b = req.body;
    ...
    const id = randomUUID();
    const created = await executeReturning(`INSERT INTO inspections (id, vehicle_id, driver_name, inspection_date, overall_status, checklist, notes, photo_count)
...
router.patch('/:id', requireRole('admin', 'manager'), requireIdParam(), asyncHandler(async (req, res) => {
    ...
    if (b.overallStatus !== undefined) {
        ...
        updates.push(`overall_status = $${idx++}`);
        params.push(b.overallStatus);
    }
    if (b.checklist !== undefined) {
        updates.push(`checklist = $${idx++}`);
        params.push(JSON.stringify(b.checklist));
    }
    if (b.notes !== undefined) {
        updates.push(`notes = $${idx++}`);
        params.push(b.notes);
    }
    if (b.photoCount !== undefined) {
        updates.push(`photo_count = $${idx++}`);
        params.push(b.photoCount);
    }
    if (updates.length === 0) {
    ...
    const updated = await executeReturning(`UPDATE inspections SET ${updates.join(', ')} WHERE id = $${idx} RETURNING ${COLUMNS_SQL}`, params);
    if (!updated) {
