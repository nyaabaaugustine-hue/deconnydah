...
router.get('/:id', requireIdParam(), asyncHandler(async (req, res) => {
    const row = await queryOne(`SELECT ${COLUMNS_SQL} FROM supervisors WHERE id = $1 AND deleted_at IS NULL`, [req.params.id]);
    if (!row) {
...
router.post('/', requireRole('admin', 'manager'), requireFields(['fullName', 'phone', 'region']), asyncHandler(async (req, res) => {
    const b = req.body;
    const id = randomUUID();
    const created = await executeReturning(`INSERT INTO supervisors (id, full_name, phone, region) VALUES ($1, $2, $3, $4)
       RETURNING ${COLUMNS_SQL}`, [id, b.fullName, b.phone, b.region]);
    res.status(201).json(created);
}));
// PATCH /api/supervisors/:id
router.patch('/:id', requireRole('admin', 'manager'), requireIdParam(), asyncHandler(async (req, res) => {
    const existing = await queryOne(`SELECT ${COLUMNS_SQL} FROM supervisors WHERE id = $1 AND deleted_at IS NULL`, [req.params.id]);
    if (!existing) {
        res.status(404).json({ error: 'Supervisor not found' });
        return;
    }
    ...
    if (fields.length > 0) {
        ...
        updated = await executeReturning(`UPDATE supervisors SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING ${COLUMNS_SQL}`, values);
    }
    else {
        updated = existing;
    }
...
router.delete('/:id', requireRole('admin'), requireIdParam(), asyncHandler(async (req, res) => {
    const result = await execute(`UPDATE supervisors SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL`, [req.params.id]);
