...
export async function authenticateRequest(req) {
    ...
    const row = await queryOne(AUTH_SQL, [token]);
    if (!row)
        return null;
    const entry = {
        userId: row.user_id,
 ...
 */
export async function authenticateRequestFull(token) {
    // Check cache first
    const cached = authCache.get(token);
    if (cached && cached.expiresAt > Date.now()) {
...
export async function seedDefaultAdmin() {
    const existing = await queryOne('SELECT id FROM admin_users WHERE username = $1', ['admin']);
    if (!existing) {
        ...
        await query('INSERT INTO admin_users (id, username, password_hash, display_name, role, must_change_password) VALUES ($1, $2, $3, $4, $5, $6)', [id, 'admin', hash, 'Administrator', 'admin', true]);
